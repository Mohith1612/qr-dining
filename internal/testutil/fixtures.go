...
func SeedFixtures(t testing.TB, pool *pgxpool.Pool) TestFixtures {
	...
	err := pool.QueryRow(ctx,
		`INSERT INTO restaurants (name, slug, settings_json) VALUES ('Test Restaurant', $1, '{}')
		 ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
		"test-restaurant-"+randomHex(4),
	).Scan(&f.RestaurantID)
	if err != nil {
	...
	err = pool.QueryRow(ctx,
		`INSERT INTO menu_categories (branch_id, name, display_order) VALUES ($1, 'Mains', 1) RETURNING id`,
		f.BranchID,
	).Scan(&categoryID)
	if err != nil {
	...
	err = pool.QueryRow(ctx,
		`INSERT INTO menu_items (category_id, name, price, is_available) VALUES ($1, 'Test Item', 10.00, true) RETURNING id`,
		categoryID,
	).Scan(&f.MenuItemID)
	if err != nil {
